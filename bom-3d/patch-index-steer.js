...
const start = s.indexOf("// 前轮转向：中置舵机");
const end = s.indexOf("reg('steer'");
if (start < 0 || end < 0) {
  console.error("markers", start, end);
  process.exit(1);
}
const repl = `// 前轮转向：中置舵机坐在底板上方 + 横拉杆 + 主销（可工作机构）
{
  const g = new THREE.Group();
  const kpX = D.wxFront;
  const wy = D.wy, wr = D.wr, ww = D.ww;
  const plateTop = D.bodyZ + 0.013;   // 底板上表面 ≈48mm
  const servoH = 0.022;               // MG996R 机体高
  const armY = plateTop + servoH + 0.006; // 舵盘/拉杆高度 ≈70mm
  const armOut = 0.040;
  const tipZ = wy - armOut;
  const servoX = kpX - 0.008;
  const servoY = plateTop + 0.002 + servoH / 2;

  // 座板坐在底板上（禁止挂到 y<bodyZ）
  ...
  g.add(box(Math.max(Math.abs(tieX - (servoX + hornLen)), 0.008), 0.004, 0.008, MAT.metal,
    (servoX + hornLen + tieX) / 2, armY, 0));
  ...
  for (const s of [1, -1]) {
    ...
    g.add(cyl(wr, ww, MAT.rubber, kpX, wr, z, Math.PI / 2, 0, 0, 48));
    for (let i = 0; i < 12; i++) {
      const a = i * Math.PI / 6;
      g.add(box(0.006, 0.004, ww * 0.9, MAT.dark,
        kpX + (wr - 0.002) * Math.cos(a), wr + (wr - 0.002) * Math.sin(a),
        z, 0, 0, a));
    }
    g.add(cyl(wr * 0.40, ww + 0.004, MAT.metal, kpX, wr, z, Math.PI / 2, 0, 0));
    g.add(cyl(0.012, ww + 0.010, MAT.bearing, kpX, wr, z, Math.PI / 2, 0, 0, 20));
    g.add(box(wr * 1.7, 0.006, ww + 0.018, MAT.petg, kpX, wr + wr + 0.004, z));
  }

  `;
s = s.slice(0, start) + repl + s.slice(end);
fs.writeFileSync(p, s, "utf8");
console.log("patched index.html steer");
