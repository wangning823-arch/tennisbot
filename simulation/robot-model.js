...
export function buildRobot() {
  ...
  // 滚轮电机 + GT2
  {
    ...
    const dx = motorX - D.rollX, dy = motorY - D.rollZ;
    const beltLen = Math.hypot(dx, dy), beltAng = Math.atan2(dy, dx);
    g.add(box(beltLen, 0.006, 0.004, MAT.rubber,
      (D.rollX + motorX) / 2, (D.rollZ + motorY) / 2 + (pulleyR1 + pulleyR2) / 2, beltZ, 0, 0, beltAng));
    g.add(box(beltLen, 0.006, 0.004, MAT.rubber,
      (D.rollX + motorX) / 2, (D.rollZ + motorY) / 2 - (pulleyR1 + pulleyR2) / 2, beltZ, 0, 0, beltAng));
    const wirePts = [
